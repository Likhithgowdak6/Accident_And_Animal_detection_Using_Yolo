...
class Toplevel1:
    def __init__(self, top=None):
        ...
        def camera_module(event):
            global program_location, algorithm
            if algorithm=="small":
                YoloVideoBased.detect(0,r"model/yolo_best_small.onnx")
            else:
                YoloVideoBased.detect(0,r"model/yolo_best.onnx")
                # Thread(target=YoloVideoBased.detect, args=(video_file_name,)).start()
        ...
        self.label_2.configure(text='''Accident & Animal Detection System ('''+algorithm+")")
